...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'submit_donation') {
    ...
    if (!$event_id) {
        ...
        foreach ($items as $idx => $item_id) {
            ...
            if ($item_id && $qty > 0) {
                mysqli_query($conn, "INSERT INTO DONATION_ITEM (donation_id, item_id, quantity) VALUES ($donation_id, $item_id, $qty) ON DUPLICATE KEY UPDATE quantity=quantity+$qty");
                mysqli_query($conn, "INSERT INTO INVENTORY (item_id, quantity) VALUES ($item_id, $qty) ON DUPLICATE KEY UPDATE quantity=quantity+$qty");
            }
        }
...
<head>
    ...
    <style>
        .donate-form label { display:block; font-weight:bold; color:#443025; margin-bottom:5px; font-size:14px; }
        .donate-form select, .donate-form input[type="number"] { padding:10px; border:2px solid #A86B6C; border-radius:10px; background:#FFE4EF; color:#443025; font-size:14px; margin-bottom:12px; outline:none; box-sizing:border-box; }
        .donate-form select { min-width:280px; }
        .donate-form input[type="number"] { width:80px; }
        .item-row { display:flex; align-items:center; gap:15px; margin-bottom:10px; flex-wrap:wrap; }
        .needed-list { background:#FFE4EF; border:1px solid #A86B6C; border-radius:10px; padding:12px 16px; margin-bottom:18px; }
        .needed-list h3 { color:#443025; margin-bottom:8px; font-size:14px; }
        .needed-list ul { margin:0; padding-left:18px; color:#443025; font-size:14px; }
